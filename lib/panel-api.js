...
export async function startPanelApi({ port, host } = {}) {
  ...
  panelServer = http.createServer(async (req, res) => {
    try {
      ...
      // ===== Soporte (Chat) =====
      // Feature removed: soporte interno por chat. Usar botón flotante (WhatsApp/Email) y/o Pedidos.
      if (pathname.startsWith('/api/support')) {
        return json(res, 410, { error: 'Soporte por chat interno deshabilitado. Usa el botón flotante (WhatsApp/Email) o crea un ticket en Pedidos.' })
      }
      ...
      if (pathname === '/api/support/my-chat' && method === 'POST') {
        ...
        try {
          ...
          const link = base ? `${base}/soporte?chat=${chat.id}` : `/soporte?chat=${chat.id}`
      ...
      if (supportMsgMatch && method === 'POST') {
        ...
        return json(res, 200, { success: true, chat })
      }
