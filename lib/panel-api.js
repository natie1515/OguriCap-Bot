      ...
      if (pathname.startsWith('/api/auth/') && (method === 'POST' || method === 'GET')) {
        try {
          ...
          if (pathname === '/api/auth/login' && method === 'POST') {
            ...
            const alertTo = safeString(process.env.SECURITY_ALERT_EMAIL_TO || process.env.SMTP_REPLY_TO || process.env.SMTP_USER || '').trim()
            const clientIp = getClientIP(req)
          ...
          if (pathname === '/api/auth/register-public' && method === 'POST') {
            ...
            const now = new Date()
            const metadata = { email: emailStr, registered_via: 'public' }
            ...
            if (db.pool?.query) {
              try {
                const result = await db.pool.query(
                  `
                  INSERT INTO usuarios (username, password, rol, activo, metadata)
                  VALUES ($1, $2, $3, $4, $5::jsonb)
                  RETURNING id, fecha_registro
                  `,
                  [usernameStr, hashed, 'usuario', true, JSON.stringify(metadata)]
                )
