...
class UserDataSynchronizer {
  constructor() {
    ...
    this.auditLogs = [];
    this._usuariosDirty = false;
  }

  markUsuariosDirty() {
    this._usuariosDirty = true;
  }

  async flushUsuarios() {
    if (!this._usuariosDirty) return;

    try {
      if (typeof this.db.writeUsuarios === 'function') {
        await this.db.writeUsuarios();
      } else if (typeof this.db.write === 'function') {
        await this.db.write();
      }
      this._usuariosDirty = false;
    } catch (error) {
